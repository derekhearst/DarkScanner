const storageFolder = import.meta.env.APPDATA + '/darkscanner/'
export type Fix = {
	id: number
	from: string
	to: string
...
async function getRarities(token: string) {
...
async function getEnchantments(token: string) {
...
async function getFixes(token: string) {
