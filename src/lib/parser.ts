...
export function parseItemDetails(text: string) {
	manualFixes.forEach((name) => (text = text.replaceAll(name.bad, name.good)))
...
export function parseAttributes(text: string) {
	...
	return Array.from(attributeMatches, (match) => {
		...
		if (!number) return
		const numberWithoutPercent = number[0].replace(/%/g, '')
		const numberWithoutDot = numberWithoutPercent.replace(/./g, '')
		const numberWithoutCommas = numberWithoutDot.replace(/,/g, '')
